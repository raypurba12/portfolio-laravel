<x-admin-layout>
    <x-slot name="header">Edit Skill</x-slot>

    <div class="max-w-2xl">
        <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between">
                <h2 class="text-sm font-semibold text-slate-700 dark:text-slate-200">Edit Skill</h2>
            ...
            <form method="POST" action="{{ route('admin.skills.update', $skill->id) }}" enctype="multipart/form-data" class="p-6 space-y-5">
                @csrf
                @method('PUT')
                    ...
                    <div>
                        ...
                        <select id="skill_category_id" name="skill_category_id"
                            ...
                            @foreach($categories as $category)
                                <option value="{{ $category->id }}" {{ old('skill_category_id', $skill->skill_category_id) == $category->id ? 'selected' : '' }}>{{ $category->name }}</option>
                            @endforeach
                        </select>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        ...
                        <x-text-input id="percentage" class="block mt-1 w-full" type="number" name="percentage" :value="old('percentage', $skill->percentage)" required min="0" max="100" />
                        <x-input-error :messages="$errors->get('percentage')" class="mt-2" />
                    </div>
                    <div>
                        <x-input-label for="order" :value="__('Display Order')" />
                        <x-text-input id="order" class="block mt-1 w-full" type="number" name="order" :value="old('order', $skill->order)" required />
                ...
                <div>
                    ...
                    <x-text-input id="color" class="block mt-1 w-full" type="text" name="color" :value="old('color', $skill->color)" placeholder="#6366F1" />
                ...
                <div>
                    ...
                    <div class="mt-2 mb-2 flex items-center gap-3">
                        <img src="{{ str_starts_with($skill->logo_path, 'http') ? $skill->logo_path : asset('storage/' . str_replace('public/', '', $skill->logo_path)) }}" alt="Current Logo"
                             class="w-12 h-12 object-contain rounded-lg bg-slate-100 dark:bg-slate-700 p-1 border border-slate-200 dark:border-slate-600">
                ...
                <div class="flex items-center gap-3 pt-1">
                    <input id="featured" type="checkbox"
                           ...
                           name="featured" value="1" {{ old('featured', $skill->featured) ? 'checked' : '' }}>
