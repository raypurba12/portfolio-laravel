<x-admin-layout>
    <x-slot name="header">Edit Background</x-slot>
    <x-admin-card class="max-w-xl">
        <form method="POST" action="{{ route('admin.hero-backgrounds.update', $heroBackground) }}" class="space-y-5">
            @csrf @method('PUT')
            ...
            <div>
                ...
                <select id="type" name="type" required
                    ...
                    <option value="image" {{ old('type', $heroBackground->type) === 'image' ? 'selected' : '' }}>Image</option>
                    <option value="video" {{ old('type', $heroBackground->type) === 'video' ? 'selected' : '' }}>Video</option>
                </select>
            ...
            <div>
                ...
                <x-text-input id="url" name="url" type="url" class="mt-1 block w-full"
                    :value="old('url', $heroBackground->url)" placeholder="https://..." required/>
                <x-input-error :messages="$errors->get('url')" class="mt-1"/>
                @if($heroBackground->type === 'image' && $heroBackground->url)
                <img src="{{ $heroBackground->url }}" alt="" class="mt-2 h-20 rounded border border-gray-600 object-cover"
            ...
            <div>
                ...
                <x-text-input id="poster" name="poster" type="url" class="mt-1 block w-full"
                    :value="old('poster', $heroBackground->poster)" placeholder="https://..."/>
            ...
            <div>
                ...
                <x-text-input id="duration" name="duration" type="number" step="0.1" class="mt-1 block w-full"
                    :value="old('duration', $heroBackground->duration ? $heroBackground->duration / 1000 : 6.5)" min="0.5" max="60"/>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <x-text-input id="order" name="order" type="number" class="mt-1 block w-full"
                        :value="old('order', $heroBackground->order)" min="0"/>
                </div>
                <div class="flex items-end pb-2">
                    <label class="flex items-center gap-2">
                        <input id="is_active" name="is_active" type="checkbox" value="1"
                               {{ old('is_active', $heroBackground->is_active) ? 'checked' : '' }}
