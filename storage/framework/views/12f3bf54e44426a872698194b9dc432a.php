...
<?php $component->withAttributes([]); ?>
     <?php $__env->slot('header', null, []); ?> Hero Backgrounds <?php $__env->endSlot(); ?>
     <?php $__env->slot('subheader', null, []); ?> Manage slideshow backgrounds for the hero section (images &amp; videos). <?php $__env->endSlot(); ?>
...
<?php endif; ?>
    ...
    <div class="flex justify-end mb-4">
        <a href="<?php echo e(route('admin.hero-backgrounds.create')); ?>"
    ...
    <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden overflow-x-auto">
        <table class="min-w-full">
            ...
            <tbody class="divide-y divide-slate-100 dark:divide-slate-700/60">
                <?php $__empty_1 = true; $__currentLoopData = $backgrounds; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $bg): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors duration-100">
                    <td class="px-5 py-3.5">
                        ...
                        <?php else: ?>
                        <img src="<?php echo e($bg->url); ?>" alt="" class="w-16 h-10 object-cover rounded-lg border border-slate-200 dark:border-slate-600 flex-shrink-0"
                             onerror="this.style.display='none'">
                        <?php endif; ?>
                    </td>
                    <td class="px-5 py-3.5">
                        <span class="inline-flex px-2.5 py-1 text-xs font-semibold rounded-full <?php echo e($bg->type === 'video' ? 'bg-indigo-100 text-indigo-700 dark:bg-indigo-500/10 dark:text-indigo-400' : 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400'); ?>">
                            <?php echo e(ucfirst($bg->type)); ?>

                        </span>
                    </td>
                    <td class="px-5 py-3.5">
                        <a href="<?php echo e($bg->url); ?>" target="_blank" class="text-sm text-indigo-600 dark:text-indigo-400 hover:underline truncate max-w-xs block"><?php echo e($bg->url); ?></a>
                        <?php if($bg->poster): ?>
                        <span class="text-[10px] text-slate-400 dark:text-slate-500">poster: <?php echo e($bg->poster); ?></span>
                        <?php endif; ?>
                    </td>
                    <td class="px-5 py-3.5 text-center">
                        <span class="inline-flex px-2.5 py-1 text-xs font-semibold rounded-full <?php echo e($bg->is_active ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400' : 'bg-slate-100 text-slate-500 dark:bg-slate-700 dark:text-slate-400'); ?>">
                            <?php echo e($bg->is_active ? 'Active' : 'Inactive'); ?>

                        </span>
                    </td>
                    <td class="px-5 py-3.5 text-center text-sm text-slate-500 dark:text-slate-400"><?php echo e($bg->order); ?></td>
                    <td class="px-5 py-3.5 text-right">
                        <div class="flex items-center justify-end gap-2">
                            <a href="<?php echo e(route('admin.hero-backgrounds.edit', $bg)); ?>"
                            ...
                            <form action="<?php echo e(route('admin.hero-backgrounds.destroy', $bg)); ?>" method="POST" class="inline">
...
<?php endif; ?>
<?php /**PATH D:\FORTOFOLIO\portfolio-laravel\resources\views/admin/hero-backgrounds/index.blade.php ENDPATH**/ ?>